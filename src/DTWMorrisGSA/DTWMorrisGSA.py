...
"""
DTWMorrisGSA: Dynamic Time Warping (DTW)-based Morris' Method of Elementary Effects computation for dynamical systems
...
def Morris_Measure_Groups(NumFact, Sample, OutFact, Output, p=4,
                          Group=[], Diagnostic=False, metric="cosine"):
    """
    ...
    if NumGroups == 0:
        ...
        OutMatrix = np.zeros((NumFact, 3))
    else:
        # for every output: every factor is a line, column is mu*
        OutMatrix = np.zeros((NumFact, 1))
        ...
        for j in range(sizea):
            if NumGroups != 0:  # work with groups
                ...
                for gk in Change_factor:
                    ...
                    # create a matrix for DTW alignment cost per factor
                    cost_d_tot = []
                    # compute DTW for each output and take mean and append
                    for d in range(x_seq.shape[1]):

                        dist_mat_d = distance.cdist(x_seq[:,d].reshape(-1,1), y_seq[:,d].reshape(-1,1), metric=metric)
                        path_d, cost_mat_d = dp(dist_mat_d)
                        cost_d_tot.append(np.nansum(cost_mat_d)) # use nanmean to force nans to be zero (zero cost)
                    
                    # compute EE by taking sum of all costs
                    SAmeas[gk, i] = (np.array(cost_d_tot).sum()/Delt)
            else:
                ...
                SAmeas[Single_Facts[j], i] = (np.array(cost_d_tot).sum()/Delt)
    ...
    OutMatrix[:NumFact, 0] = AbsMu
    if NumGroups == 0:
        OutMatrix[:NumFact, 1] = Mu
        OutMatrix[:NumFact, 2] = Stdev

    SAmeas_out[:NumFact, :] = SAmeas
...
def elementary_effects(nparam, OptMatrix, OptOutVec, Output,
                       nsteps=4, Group=[], Diagnostic=False, metric="cosine"):
    """
    ...
    return Morris_Measure_Groups(nparam, OptMatrix, OptOutVec,
                                 Output, nsteps, Group, Diagnostic, metric)
